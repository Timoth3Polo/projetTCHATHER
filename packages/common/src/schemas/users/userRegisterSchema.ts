...
export const userRegisterSchema = yup.object().shape({
    ...
    userPassword: yup.string().required(REQUIRED_ERROR_MESSAGE).matches(/^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$ %^&*-]).{8,}$/, "Le mot de passe doit contenir un caractère spécial, une lettre majuscule, minuscule et un chiffre"),
